...
- Данные хранятся в store и доступны через REST API (`/api/conversations`, `/api/messages`), берутся только нужные поля из мок даты.
...
- Избегать useContext для глобального состояния
